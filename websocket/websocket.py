...
def InitWebSocketServer():
    ...
    try:
        ...
        print("Server is already runing ,quit")
        sys.exit()
    while True:
        connection, address = sock.accept()
        if(handshake(connection) != False):
            t = threading.Thread(target=DoRemoteCommand, args=(connection, files))
            t.start()
...
def handshake(client):
    ...
    szHost = headers["Host"]
    our_handshake = "HTTP/1.1 101 Switching Protocol\r\n" \
                    "Upgrade:websocket\r\n" \
                    "Connection: Upgrade\r\n" \
...
def DoRemoteCommand(connection, files):
    for line in tailer.follow(open(files)):
        SendData(line, connection)
